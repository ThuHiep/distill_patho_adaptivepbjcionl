...
## ▶️ TRẠNG THÁI / NEXT (khi user nói "tiếp tục" → làm bước ⬜ đầu tiên ở đây)

- [ ] **Bước 1 — UQ-floor 5-seed** (đang chạy trên vast lần cuối; kiểm log, gom số → §4.3, backup pkl)
- [ ] **Bước 2 — Ablations leak-free** (nhẹ, cùng density cache)
- [ ] **Bước 3 — KD 5-seed + significance** (nặng: `setup_pathosam_vast.sh` + raw NuInsSeg) → LÕI C1/N4
- [ ] **Bước 4 — MoNuSAC transfer** (cần PanNuke cache 3.6GB + disk ≥80GB)
- [ ] **Bước 7 — Viết manuscript** (chỉ khi §4.1/4.3/4.8 = ✅ verified hết)

> **Quy tắc BẮT BUỘC mỗi bước** ([[feedback-note-backup-results]]): pkl/cache vừa sinh → (a) ghi provenance + số vào
> PAPER2_MASTER.md NGAY, (b) `kaggle datasets version` NGAY. Trước khi chạy gì → check đã có chưa (§0) để khỏi trùng.
...
> Chuỗi: `prep_monusac_counts.py` (pkl→ảnh+csv) → `eval_cross_dataset.py` (student PanNuke→predict MoNuSAC OOD) → `eval_r2_grouped.py`.
> MoNuSAC KHÔNG có nhãn organ (chỉ 4 class Epithelial/Lymphocyte/Macrophage/Neutrophil) → **chỉ report marginal coverage/Winkler**, KHÔNG grouping organ (dùng `--pit_scheme global`). Ghi trung thực.

```bash
# 4a. upload monusac_converted.pkl từ MAC (chạy Ở MAC):
...
# 4b. prep: pkl → folder ảnh PNG + gt_counts.csv
python prep_monusac_counts.py --pkl ../data/monusac_converted.pkl --out ../work/monusac_png

# 4c. transfer: train student trên PanNuke → predict MoNuSAC (OOD, total-count K=1)
python eval_cross_dataset.py --train_dataset pannuke --detach_mu \
  --test_images_dir ../work/monusac_png/images \
  --test_gt_csv ../work/monusac_png/gt_counts.csv \
  --out ../work/xfer_pannuke2monusac.pkl 2>&1 | tee -a /tmp/monusac.log

# 4d. eval (global — MoNuSAC không có organ label)
python eval_r2_grouped.py --preds ../work/xfer_pannuke2monusac.pkl --pit_scheme global \
  2>&1 | tee -a /tmp/monusac.log
grep -Ei "marg|winkler|cov|mae" /tmp/monusac.log

# 4e. backup NGAY
kaggle datasets version -p ../work -m "add MoNuSAC transfer pkl"
```
> Cần PanNuke train → nhớ kéo `teacher_density_pannuke_f123.pkl` (3.6GB) + đủ disk (≥80GB) cho bước này.
...
## BƯỚC 7 — Viết manuscript (khi số đã ✅ verified hết)

Đóng gói **"Distributional Count Distillation under mean–variance optimization conflict"**. KHÔNG claim PB-JCI/online (=Paper 1).
Cấu trúc + điểm phải có:
- **Method → Architecture choice:** 1 đoạn justify backbone U-Net (task density-counting cần decoder giữ độ phân giải;
  encoder swappable nhưng decoder U-Net không; backbone ≠ novelty; ch32 sweet spot có sweep; FastViT = future work). Cite CSRNet/U-Net.
- **Novelty:** N1 tổ hợp, N2 Poisson-σ, N3 detach_mu, N4 learned>score under compression, N5 coverage transfers.
- **Framing trung thực (quy tắc 7):** "reliability ngang tầm UQ ở giá rẻ nhất" (KHÔNG "best UQ"); MAE không transfer;
  annotation-cost caveat; NuInsSeg multi-seed.
- **3 trụ** (label-efficient distill / distributional UQ / efficiency). Accuracy = "cạnh tranh cho ngân sách nhãn".
- **Hình ~4–5:** (1) kiến trúc + 2 head; (2) σ Poisson-anchor diagnostic→fix (corr −0.02→+0.40); (3) coverage curve/reliability;
  (4) transfer cross-dataset; (5) efficiency vs UQ scatter.
...
5. → **Bước 7 viết manuscript** khi §4.1/§4.3/§4.8 đã "✅ verified" hết.

**Quyết định đã CHỐT (2026-07-17):** C3 = **GIỮ CondConf/PCP** (bằng chứng có lợi: R2 đè PCP cả 2 trục, thắng CondConf
Winkler; reviewer đòi; không đụng Paper 1). Không còn quyết định treo.
