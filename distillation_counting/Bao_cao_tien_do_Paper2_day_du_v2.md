...
|---|---|---|---|---|---|
| PathoSAM teacher | ~640M | *(cần đo)* | ~2.56 GB | *(cần đo)* | *(cần đo)* |
| **PACT (ch32, chính)** | **1.935M** | 10.49 | 7.74 MB | **4.91** | **70.7 MB** |

PACT **1.935M** — nhỏ hơn teacher (~640M) ~**330× tham số**, file **~330× nhẹ hơn** (7.74 MB vs ~2.56 GB); cũng nhỏ hơn student H-Optimus (24M) ~**12×**, NuLite (12M) ~**6×**. Đo thực trên **Tesla T4**: chỉ **4.91 ms/ảnh (204 ảnh/giây)** và **70.7 MB VRAM** — đủ chạy trên GPU/CPU khiêm tốn (đúng động cơ deployment như H-Optimus). *(Latency/VRAM của teacher chưa đo — cần load PathoSAM; không điền số ước lượng.)*
