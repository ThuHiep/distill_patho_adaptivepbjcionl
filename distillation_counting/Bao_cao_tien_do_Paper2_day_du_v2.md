...
| PACT (ch16, ablation) | 0.485M | 2.65 | 1.94 MB | 2.33 | 38.1 MB |

PACT **1.935M** — nhỏ hơn student H-Optimus (24M) ~**12×**, nhỏ hơn NuLite (12M) ~**6×**, teacher (640M) ~**330×**. Đo thực trên **Tesla T4**: chỉ **4.91 ms/ảnh (204 ảnh/giây)** và **70.7 MB VRAM** — so với teacher cần ~16 GB thì **nhỏ hơn ~230× về bộ nhớ**, đủ chạy trên GPU/CPU khiêm tốn (đúng động cơ deployment như H-Optimus 16GB→3GB). Cấu hình ch16 (~0.5M) không mất chất lượng (NuInsSeg nhỉnh, PanNuke hòa) → phương pháp bền theo dung lượng.
